...
const GetCourses = (props) => {
    ...
    const { isLoading, courses, asyncStart, asyncEnd } = props
    ...
    useEffect(() => {
        (async () => {
            ...
            if (query) {
                ...
                const filtedData = courses.reduce((acc, cur) => {
                ...
                const filted = courses.filter(course => course.catagories[1] === subcatagory && course.catagories[2] === topic)
    ...
    }, [catagory, subcatagory, topic, query])
